...
static const char *TAG_BT_MG = "esp32_bt_manager";
...
uint8_t base_mac_addr[6]                           = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static const uint8_t  C_PROP_READ_WRITE_NOTIFY     = ESP_GATT_CHAR_PROP_BIT_READ | ESP_GATT_CHAR_PROP_BIT_NOTIFY;


static const uint8_t heart_measurement_ccc[2]      = {0x00, 0x00};


uint16_t gatt_table[HRS_CONFIG_NB];
...
static const esp_gatts_attr_db_t gatt_db[HRS_CONFIG_NB] =
{
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_DEVICE_NAME, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_CHAR_VAL_LEN_MAX, sizeof(BT_DEVICE_NAME), (uint8_t *)BT_DEVICE_NAME}},
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_DEVICE_VERSION, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_CHAR_VAL_LEN_MAX, sizeof(MIHOME_VERSION), (uint8_t *)MIHOME_VERSION}},
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_DEVICE_BLE_ADDR, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_CHAR_VAL_LEN_MAX, sizeof(base_mac_addr), (uint8_t *)base_mac_addr}},
...
static const esp_gatts_attr_db_t gatt_db2[HRS_CONFIG_NB] = {
    ...
    [CONFIG_CHAR_READY]     =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&C_DECLARATION_UUID, ESP_GATT_PERM_READ,
      CHAR_DECLARATION_SIZE, CHAR_DECLARATION_SIZE, (uint8_t *)&C_PROP_READ_WRITE_NOTIFY}},
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_CONFIG_READY, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_CHAR_VAL_LEN_MAX, sizeof(ready_value), (uint8_t *)ready_value}},
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_CONFIG_SSID, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_CHAR_VAL_LEN_MAX, sizeof(uuid_value), (uint8_t *)uuid_value}},
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_CONFIG_PWD, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_CHAR_VAL_LEN_MAX, sizeof(password_value), (uint8_t *)password_value}},
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_CONFIG_URL, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_CHAR_VAL_LEN_MAX, sizeof(sys_url_value), (uint8_t *)sys_url_value}},
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_CONFIG_UUID, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_CHAR_VAL_LEN_MAX, sizeof(sys_uuid_value), (uint8_t *)sys_uuid_value}},
};
...
void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            ...
            ESP_LOGI(TAG_BT_MG, "APP_ID: %04x, Status: %d", param->reg.app_id, param->reg.status);
            return;
        }
    }
    do {
...
/*
    BLE GATTS Profile Event Handler
*/
void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ...
            if (set_dev_name_ret){
                ESP_LOGE(TAG_BT_MG, "esp_ble_gap_set_device_name, error code = %x", set_dev_name_ret);
            }

            esp_err_t raw_adv_ret = esp_ble_gap_config_adv_data_raw(raw_adv_data, sizeof(raw_adv_data));
            if (raw_adv_ret){
                ESP_LOGE(TAG_BT_MG, "esp_ble_gap_config_adv_data_raw, error code = %x ", raw_adv_ret);
            }
            adv_config_done |= adv_config_flag;

            esp_err_t raw_scan_ret = esp_ble_gap_config_scan_rsp_data_raw(raw_scan_rsp_data, sizeof(raw_scan_rsp_data));
            if (raw_scan_ret){
                ESP_LOGE(TAG_BT_MG, "esp_ble_gap_config_scan_rsp_data_raw, error code = %x", raw_scan_ret);
            }
            adv_config_done |= scan_rsp_config_flag;
 
            esp_err_t create_attr_ret1 = esp_ble_gatts_create_attr_tab(gatt_db, gatts_if, HRS_DEVICE_NB, 0);
            if (create_attr_ret1){
                ESP_LOGE(TAG_BT_MG, "esp_ble_gatts_create_attr_tab, error code = %x", create_attr_ret1);
            }
            esp_err_t create_attr_ret2 = esp_ble_gatts_create_attr_tab(gatt_db2, gatts_if, HRS_CONFIG_NB, 1);
            if (create_attr_ret2){
                ESP_LOGE(TAG_BT_MG, "esp_ble_gatts_create_attr_tab, error code = %x", create_attr_ret2);
            }
            break;
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:{
            if (param->add_attr_tab.status != ESP_GATT_OK) {
                ESP_LOGE(TAG_BT_MG, "ESP_GATTS_CREAT_ATTR_TAB_EVT_0, error code = 0x%x", param->add_attr_tab.status);
            } else {
                ESP_LOGI(TAG_BT_MG, "ESP_GATTS_CREAT_ATTR_TAB_EVT_1, handle = %d", param->add_attr_tab.num_handle);
                memcpy(gatt_table, param->add_attr_tab.handles, sizeof(gatt_table));
                esp_ble_gatts_start_service(gatt_table[0]);
            }
            break;
        }
        case ESP_GATTS_CREATE_EVT:
            ESP_LOGI(TAG_BT_MG, "ESP_GATTS_CREATE_EVT, status %d, service_handle %d, service_id %d",
        ...
        case ESP_GATTS_WRITE_EVT: {
           ...
           if (!param->write.is_prep){
                ...
                if (gatt_table[CONFIG_CHAR_CFG_READY] == param->write.handle && param->write.len == 2){
                    ...
                    if (descr_value == 0x0001){
                        ...
                        esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, gatt_table[CONFIG_CHAR_VAL_READY],
                   ...
                   // esp_log_buffer_hex(TAG_BT_MG, indicate_data, sizeof(indicate_data));
                    ...
                    esp_ble_gatts_set_attr_value(gatt_table[CONFIG_CHAR_VAL_READY], param->write.len,  param->write.value);
        ...
        case ESP_GATTS_ADD_CHAR_DESCR_EVT:
            ...
            ESP_LOGI(TAG_BT_MG, "ESP_GATTS_ADD_CHAR_DESCR_EVT, status %d, attr_handle %d, service_handle %d", param->add_char_descr.status, param->add_char_descr.attr_handle, param->add_char_descr.service_handle);
            break;
